...
class UserAdapter(val listener : (UserItem) -> Unit) : RecyclerView.Adapter<UserAdapter.UserViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): UserViewHolder {
        ...
        return UserViewHolder(convertView = convertView)
    }
    ...
    inner class UserViewHolder(val convertView:View) : RecyclerView.ViewHolder(convertView), LayoutContainer {
        override val containerView: View?
            get() = itemView

        fun bind(user:UserItem, litener: (UserItem)->Unit) {
            if (user.avatar!=null) {
                Glide.with(itemView)
                    .load(user.avatar)
                    .into(iv_avatar_user)
